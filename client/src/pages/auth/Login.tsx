...
export default function Login({ isModal, onSuccess, onSwitchToRegister }: LoginProps) {
  ...
  const content = (
    <>
      ...
      <div className="flex bg-gray-100 rounded-lg p-1 mb-5">
        ...
        >
          <Smartphone size={16} /> {t('auth.phoneNumber')}
        </button>
        <button
        ...
        >
          <span className="font-bold">@</span> Email
        </button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-xs text-gray-500 mb-1 uppercase tracking-wider font-medium">
            {loginMethod === 'phone' ? t('auth.phoneNumber') : 'Email'}
          </label>
          <input
            type={loginMethod === 'phone' ? 'text' : 'email'}
            value={phone}
            onChange={(e) => setPhone(e.target.value)}
            placeholder={loginMethod === 'phone' ? '+251 912 345 678' : 'example@gmail.com'}
            className="w-full bg-white border border-gray-300 rounded-lg px-4 py-3 text-gray-800 placeholder-gray-400 focus:border-[#00A8FF] focus:outline-none focus:ring-2 focus:ring-[#00A8FF]/10"
          />
        </div>
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              placeholder="••••••••"
              className="w-full bg-white border border-gray-300 rounded-lg px-4 py-3 text-gray-800 placeholder-gray-400 focus:border-[#00A8FF] focus:outline-none focus:ring-2 focus:ring-[#00A8FF]/10 pr-12"
            />
            <button
              type="button"
        ...
        <button type="submit" disabled={loading} className="w-full bg-[#00A8FF] hover:bg-[#0090DD] text-white font-semibold py-3 rounded-lg transition-all shadow-md shadow-[#00A8FF]/20">
  ...
  // If used as modal, just return content without wrapper
